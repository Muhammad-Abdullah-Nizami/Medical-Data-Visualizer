...
def draw_cat_plot():
    ...
    df_cat = pd.melt(df, value_vars=['cholesterol', 'gluc', 'smoke', 'alco', 'active', 'overweight']).sort_values('variable')
...
def draw_heat_map():
    ...
    sns.heatmap(corr, mask=mask, vmax=.3, square=True, linewidths=.5, annot = True, fmt='.1f')
    # Do not modify the next two lines
